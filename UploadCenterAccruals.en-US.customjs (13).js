(function (w, d) {
  ...
  // Currencies loaded from admin-managed Dataverse table (cr650_dclcurrencieses)
  const CURRENCY_API_URL = '/_api/cr650_dclcurrencieses?$select=cr650_currencycode,cr650_currencysymbol,cr650_currencyname,cr650_sortorder';
  let DCL_STATUS = null;
  ...
  /* =========================================================
   * SHIPMENT MONTH (DCL-level field on cr650_dcl_masters.cr650_shipmentmonth)
   * The value surfaces in the Shipment Month column of BOTH the Summary Accruals
   * and Expense Accruals reports. When set, it overrides the legacy derivation
   * from cr650_dcl_shipped_orderses.cr650_shipment_date in the report view.
   * ========================================================= */
  const SHIPMENT_MONTH_FIELD = "cr650_shipmentmonth";

  async function loadShipmentMonth(masterId) {
    const sel = d.getElementById("shipmentMonthSelect");
    if (!sel || !masterId) return;
    try {
      const r = await webapi.safeAjax({
        type: "GET",
        url: `${DCL_API_BASE}(${masterId})?$select=${SHIPMENT_MONTH_FIELD}`,
        contentType: "application/json"
      });
      sel.value = r?.[SHIPMENT_MONTH_FIELD] || "";
    } catch (e) {
      console.warn("[Shipment Month] Could not load existing value:", e);
    }
  }

  function setShipmentMonthStatus(state) {
    const el = d.getElementById("shipmentMonthStatus");
    if (!el) return;
    // Reset state classes; keep base class only
    el.className = "dcl-meta-status";
    el.removeAttribute("title");
    if (state === "saving") {
      el.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
      el.classList.add("is-visible", "is-saving");
      el.title = "Saving…";
    } else if (state === "saved") {
      ...
      el.title = "Save failed — try again";
    } else {
      el.innerHTML = "";
    }
  }

  async function saveShipmentMonth(masterId, value) {
    if (!masterId) return;
    setShipmentMonthStatus("saving");
    try {
      const payload = {};
      // Empty string → null so Dataverse clears the field instead of storing "".
      // Normalize to UPPERCASE to match extractMonth() output ("JAN".."DEC") so
      // the Monthly Expense Accruals month filter compares apples to apples.
      const normalized = value ? String(value).trim().toUpperCase() : null;
      payload[SHIPMENT_MONTH_FIELD] = normalized;
      ...
      setShipmentMonthStatus("saved");
      setTimeout(() => setShipmentMonthStatus(null), 2000);
    } catch (e) {
      console.error("[Shipment Month] Save failed:", e);
      setShipmentMonthStatus("error");
    }
  }

  function wireShipmentMonth(masterId) {
    const sel = d.getElementById("shipmentMonthSelect");
    if (!sel || !masterId || sel.dataset.bound === "1") return;
    sel.addEventListener("change", () => saveShipmentMonth(masterId, sel.value));
    sel.dataset.bound = "1";
  }

  // Loaded once per page session from cr650_documents_masters. No cross-session cache,
  // so admin edits to the Document Masters table are picked up on the next page reload.
  // The in-memory `docOptionsLoaded` flag prevents re-fetching during a single session.
  let DOC_OPTIONS = [];
  let docOptionsLoaded = false;
  async function ensureDocOptionsLoaded() {
    if (docOptionsLoaded) return;
    try {
      ...
      rebuildUniqueTypes();
      docOptionsLoaded = true;
    } catch {
      DOC_OPTIONS = [{ value: "other-doc", label: "Other Documents" }];
      rebuildUniqueTypes();
      // leave docOptionsLoaded = false so the next call retries the network fetch
    }
  }
    ...
    try {
      ...
      const list = (data.value || [])
        .map(r => ({
          ...
          symbol: String(r.cr650_currencysymbol || '').trim(),
      ...
      console.warn('Could not load currencies from admin table, using fallback:', err);
      CURRENCIES = fallback;
    }
  }

  // Unified currency option renderer — symbol (if any) + ISO code + em dash + name.
  // Identical to the helper in Customer-Management, Create-New-DCL, and
  // Basic_Information_View so every dropdown looks the same across the system.
  function formatCurrencyOption(c) {
    const code = escapeHtml(c.code || '');
    const name = escapeHtml(c.name || '');
    const sym = (c.symbol || '').trim();
    const symPrefix = sym ? escapeHtml(sym) + '&nbsp;' : '';
    return `<option value="${escapeHtml(c.code || '')}">${symPrefix}${code} — ${name}</option>`;
  }
  ...
  function populateCurrencySelect(sel) {
    ...
    CURRENCIES.forEach(c => options.push(formatCurrencyOption(c)));
  ...
  function lockFormIfSubmitted() {
    try {
      ...
      // 8. Disable shipment month dropdown (DCL-level field)
      const shipmentSel = d.getElementById("shipmentMonthSelect");
      if (shipmentSel) {
        shipmentSel.disabled = true;
        shipmentSel.style.cursor = "not-allowed";
        shipmentSel.style.opacity = "0.6";
      }
  ...
  async function initOnce() {
    ...
    try {
      ...
      if (masterId) {
        await ensureSystemInvoiceMapLoaded(masterId);
        await loadShipmentMonth(masterId);
        wireShipmentMonth(masterId);
      }
